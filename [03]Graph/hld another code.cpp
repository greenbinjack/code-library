...
void dfs_hld(int u, int p) {
  st[u] = ++clk;                                       /* /// put stuff in dfarr here */
...
// process node u upto it’s ancestor a // if excl is
true, a will not be processed */
int chainProcess(int a, int u, bool excl = false) {
  for (; head[u] != head[a]; u = par[head[u]]) {
    func(st[head[u]], st[u]); /* //
processing // query(1, 1, n, st[head[u]], st[u]) */
  }
  func(st[a] + excl, st[u]); /* // processing */
}                            /* //
process path from node u to node v, if order matters
will be tough // if excl is true lca will not be
processed */
