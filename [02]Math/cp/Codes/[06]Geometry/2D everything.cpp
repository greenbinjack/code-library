...
class circle {
  ...
  friend tuple <int, point <DT>, point <DT>> IntersectionPoint(const circle &a,const circle &b) {
    if(a.R == b.R and a.O == b.O) return {-1, {0, 0}, {0, 0}};
    double d = sqrt(DisSq(a.O, b.O));
    if(d > a.R + b.R or d < fabs(a.R - b.R)) return {0, {0, 0}, {0, 0}};
...
namespace polygon_algo{
  ...
  template <typename DT> int PointInConvexPolygon(polygon <int> :: iterator b, polygon <int> :: iterator e, const point <DT> &O){
    ...
    if (ans == e or abs(UTriArea(*b, *(ans - 1), *ans) - UTriArea(*b, *(ans - 1), O) - UTriArea(*b, *ans, O) - UTriArea(*(ans - 1), *ans, O)) > EPS) return 0;
  ...
  template <typename DT> int PointInPolygon(polygon <DT> &P, point <DT> pt) {
    ...
    for(int i = 0, j = 1; i < n; i++, j = (j + 1) % n) {
      if(TriArea(pt, P[i], P[j]) == 0 and min(P[i], P[j]) <= pt and pt <= max(P[i], P[j])) return -1;
...
DT Closest_Distance(vector <point <DT>> &v) {
  ...
  for (int i = 0; i < n; i++) {
    ...
    for (auto it = it1; it != it2; it++) best = min(best, Dis(v[i], *it));
    s.insert(v[i]);
  }
  return best;
}
